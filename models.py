...
class AdaptiveLSTMCell(nn.Module):
    def __init__(self, inputSize, hiddenSize):
        ...
        self.w_ih = nn.Parameter(torch.Tensor(5 * hiddenSize, inputSize))  # 5?
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, spatial_image, global_image, encoded_captions, caption_lengths, enc_image):
        ...
        # Create tensors to hold word prediction scores,alphas and betas
        ...
        for timestep in range(max(decode_lengths)):
            ...
            # First, keep a copy of the hidden state since we need to pass it to the sentinel later.
